...
const OwnerMaintenancePage = () => {
  ...
  const maintenanceColumns = [
    {
      ...
      render: (_, item) => (
        <div>
          ...
          <div className="text-sm text-gray-500">{item.property_name} - {item.unit_number}</div>
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      title: 'Assigned To',
      render: (_, item) => (
        <div className="text-sm text-gray-900">{item.assigned_to || 'Unassigned'}</div>
      )
    },
    {
      key: 'cost',
    ...
    },
    {
      ...
      title: 'Created',
      render: (_, item) => (
        <div className="text-sm text-gray-500">{formatDate(item.created_date)}</div>
      )
    },
    {
      key: 'actions',
  ...
  const currentData = activeTab === 'maintenance' ? maintenanceData : complaintsData;
  const currentColumns = activeTab === 'maintenance' ? maintenanceColumns : complaintsColumns;

  const filteredData = currentData.filter(item => {
    const matchesSearch = item.issue?.toLowerCase().includes(filters.search.toLowerCase()) ||
                         item.subject?.toLowerCase().includes(filters.search.toLowerCase()) ||
                         item.property_name?.toLowerCase().includes(filters.search.toLowerCase());
    const matchesStatus = !filters.status || item.status === filters.status;
    const matchesPriority = !filters.priority || item.priority === filters.priority;
    const matchesProperty = !filters.property || item.property_name?.includes(filters.property);
  ...
  const stats = {
    ...
    pending_maintenance: maintenanceData.filter(m => m.status === 'pending').length,
    in_progress_maintenance: maintenanceData.filter(m => m.status === 'in_progress').length,
    total_complaints: complaintsData.length,
    open_complaints: complaintsData.filter(c => c.status === 'open').length,
    resolved_complaints: complaintsData.filter(c => c.status === 'resolved').length
  };
    ...
    return (
      <div className="space-y-6">
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">Error Loading Maintenance Data</h3>
  ...
  return (
    <div className="space-y-8 animate-fade-in">
        ...
        <Card className="text-center bg-gradient-to-br from-emerald-50 to-green-50">
          <div className="text-2xl font-bold text-emerald-600">
            {loading ? (
              ...
              Math.round((stats.resolved_complaints / (stats.total_complaints || 1)) * 100)
            )}%
          </div>
        ...
        <Card className="text-center bg-gradient-to-br from-amber-50 to-orange-50">
          <div className="text-2xl font-bold text-amber-600">
            {loading ? (
              ...
              maintenanceData.reduce((sum, item) => sum + (item.estimated_cost || 0), 0).toLocaleString()
            )}
          </div>
      ...
      {/* Data Table */}
      <Card header={`${activeTab === 'maintenance' ? 'Maintenance Requests' : 'Complaints'} (${filteredData.length})`}>
        <Table
          columns={currentColumns}
          data={filteredData}
          loading={loading}
          emptyMessage={`No ${activeTab === 'maintenance' ? 'maintenance requests' : 'complaints'} found`}
          variant="default"
        />
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card header="Vendor Management" className="bg-gradient-to-br from-purple-50 to-indigo-50">
          <div className="space-y-3">
            <Button variant="primary" fullWidth>
              Manage Vendors
            </Button>
        ...
        <Card header="Analytics" className="bg-gradient-to-br from-emerald-50 to-green-50">
          <div className="space-y-3">
            <Button variant="primary" fullWidth>
              Maintenance Trends
            </Button>
