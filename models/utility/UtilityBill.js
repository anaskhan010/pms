...
class UtilityBill {
  ...
  // Get bill details with related data
  async getDetails() {
    const sql = `
      SELECT 
        ub.*,
  ...
  // Generate bills for all units for a utility type
  static async generateMonthlyBills(utilityTypeId, billDate, dueDate) {
    // Get all active meters for this utility type
    const sql = `
    ...
    for (const meter of meters) {
      ...
      const billData = {
        ...
        current_reading: previousReading, // Will be updated when actual reading is taken
        consumption: 0,
  ...
  // Update reading and recalculate amount
  ...
  // Get consumption history for a unit
