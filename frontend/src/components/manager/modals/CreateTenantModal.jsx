...
/**
 * CreateTenantModal - improved layout & responsiveness
 */
const CreateTenantModal = ({ isOpen, onClose, onTenantCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const loadNotif = notificationService.loading("Creating tenant...");
    try {
      const res = await managerApiService.createTenant(formData);
      notificationService.removeNotification(loadNotif);
      if (res.success) {
        notificationService.success("Tenant created!");
        onTenantCreated(res.data);
        handleClose();
      } else throw new Error(res.error);
    } catch {
      notificationService.error("Error creating tenant");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="fixed inset-0 bg-black bg-opacity-60 flex items-start justify-center overflow-y-auto z-50 pt-10 pb-10">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl p-4 sm:p-6 md:p-8 border border-black">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-black">Create New Tenant</h2>
            <button
              onClick={handleClose}
