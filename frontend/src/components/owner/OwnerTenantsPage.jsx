...
const OwnerTenantsPage = () => {
  ...
  // Memoized filtered tenants to prevent unnecessary re-renders
  const filteredTenants = useMemo(() => {
    return tenants.filter((tenant) => {
      ...
      const matchesPaymentStatus =
        ...
        tenant.payment_status === filters.paymentStatus;

      return matchesSearch && matchesProperty && matchesPaymentStatus;
    });
  }, [tenants, filters.search, filters.property, filters.paymentStatus]);
  ...
  const currentTenants = filteredTenants.filter(
    (t) => t.payment_status === "current"
  ).length;
  const overdueTenants = filteredTenants.filter(
    (t) => t.payment_status === "overdue"
  ).length;
  const totalRent = filteredTenants.reduce(
          ...
          <div className="space-y-3 text-sm">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-600">
                Payment received from Sarah Al-Mansouri
              </span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-2 h-2 bg-amber-500 rounded-full"></div>
              <span className="text-gray-600">
                Lease renewal due for Mohammed Al-Ahmed
              </span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-2 h-2 bg-red-500 rounded-full"></div>
              <span className="text-gray-600">
                Payment overdue from Abdullah Trading Co.
              </span>
            </div>
          </div>
        </Card>
