...
const OwnerPropertiesPage = () => {
  ...
  const handlePropertyAdded = (newProperty) => {
    ...
    // Optionally reload all properties to ensure data consistency
    // loadProperties();
  };
  ...
  const totalValue = properties.reduce(
    (sum, property) => sum + (property.property_value || 0),
    0
  );
  const totalIncome = properties.reduce(
    (sum, property) => sum + (property.monthly_income || 0),
    0
  );
  const totalUnits = properties.reduce(
    (sum, property) => sum + (property.total_units || 0),
    0
  );
  const occupiedUnits = properties.reduce(
    (sum, property) => sum + (property.occupied_units || 0),
    0
  );
